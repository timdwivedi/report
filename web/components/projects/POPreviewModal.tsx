...
function generatePONumber(): string {
  return `PO-2026-${String(Math.floor(Math.random() * 900) + 100).padStart(3, '0')}`
}
...
export default function POPreviewModal({
  ...
  const poNumber = useRef(generatePONumber()).current

  // Calculate line items with spoilage
  const poLines = lineItems.map((li) => {
    const qty = li.total_quantity ?? 0
    const spoilage = Math.max(2, Math.ceil(qty * 0.05))
    const totalQty = qty + spoilage
    const unitCost = li.unit_cost ?? 0
    const lineTotal = totalQty * unitCost
    return { ...li, qty, spoilage, totalQty, unitCost, lineTotal }
  })

  const subtotal = poLines.reduce((sum, li) => sum + li.lineTotal, 0)
  const taxRate = 0.0
  const total = subtotal * (1 + taxRate)

  // Ship-to address: prefer shipping_address, fall back to billing_address
  const shipAddress = client?.shipping_address ?? client?.billing_address
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .po-title h2 { font-size: 18px; font-weight: 600; color: #1e293b; }
          .po-title p { font-size: 12px; color: #64748b; margin-top: 4px; }
          ...
          .notes-section { margin-top: 24px; padding: 16px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; }
          .notes-section h4 { font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; color: #94a3b8; margin-bottom: 8px; }
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-slate-50 flex-shrink-0">
              <div>
                <h3 className="text-sm font-semibold text-slate-900">Purchase Order Preview</h3>
                <p className="text-xs text-slate-500">{poNumber} — {project.name}</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 bg-slate-100">
              ...
              >
                ...
                <div className="flex justify-between items-start mb-8 pb-6" style={{ borderBottom: '2px solid #2563eb' }}>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      {poNumber} &bull; {fmtDate(project.created_at)}
                    </p>
                  </div>
                </div>

                {/* Info Grid — 3 columns: Ship To, Vendor, Project Details */}
                <div className="grid grid-cols-3 gap-6 mb-8">
                  <div>
                    <h4 className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 mb-2">Ship To</h4>
                    <p className="text-sm font-semibold text-slate-800">{client?.company_name ?? project.client_name}</p>
                    {shipAddress && (
                      <>
                        <p className="text-xs text-slate-500 mt-1">{shipAddress.street}</p>
                        <p className="text-xs text-slate-500">{shipAddress.city}, {shipAddress.state} {shipAddress.zip}</p>
                      </>
                    )}
                    {client?.contacts?.[0] && (
                      <p className="text-xs text-slate-500 mt-1">Attn: {client.contacts[0].name}</p>
                    )}
                  </div>
                  <div>
                    <h4 className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 mb-2">Vendor</h4>
                    <p className="text-sm font-semibold text-slate-800">Culture Studio</p>
                    <p className="text-xs text-slate-500 mt-1">Nashville, TN</p>
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">Project: {project.project_number}</p>
                    ...
                    {poLines.map((li) => {
                      ...
                      return (
                        <tr key={li.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                          ...
                          <td className="text-center text-sm font-mono text-slate-500 px-3 py-3">
                            +{li.spoilage}
                          </td>
                          <td className="text-center text-sm font-mono font-semibold text-slate-700 px-3 py-3">
                            {li.totalQty.toLocaleString()}
                          </td>
                          <td className="text-right text-sm font-mono text-slate-600 px-3 py-3">
                            {fmtCurrency(li.unitCost)}
                          </td>
                ...
                <div className="mt-6 p-4 rounded-lg" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
                  <h4 className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 mb-2">Special Instructions</h4>
                  <p className="text-xs text-slate-600 leading-relaxed">
                    Standard production timeline. Contact for rush options.
                  </p>
                </div>

                {/* Footer */}
                <div className="mt-10 pt-6 text-center text-xs text-slate-400" style={{ borderTop: '1px solid #e2e8f0' }}>
                  <p>85 Supply &mdash; Purchase Order</p>
